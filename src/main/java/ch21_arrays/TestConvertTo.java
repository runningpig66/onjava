 ...
 * @date 2026-03-23
 * @time 21:04
 * P.786 §21.10 泛型和基本类型数组
 * <p>
 * 下面是对 ConvertTo.java 中所有方法的测试代码：在每个测试用例中，原始数组被创建为包装类型，并且像在 TestCounter.java 中一样，
 * 使用 Arrays.setAll() 填充了元素（这也验证了 Arrays.setAll() 适用于 Integer、Long 和 Double 包装类型的数组）。
 * 然后 ConvertTo.primitive() 可以将包装类数组转换成对应的基本类型数组，而 ConvertTo.boxed() 则负责反向转换。
 */
