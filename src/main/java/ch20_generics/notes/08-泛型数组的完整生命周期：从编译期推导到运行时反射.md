...
* **构造参数的约束校验：
    ...
    * **类型参数的一致性合一：
        编译器将形参 `Class<T>` 与实参 `Class<String>` 进行**模式匹配**。通过对比发现，为了满足参数传递的合法性，右侧表达式中的泛型参数 `T` 必须被解析为 `String`。
    * **最终判定**：
        编译器将右侧推导出的类型 (`T=String`) 与左侧锚定的目标类型 (`T=String`) 进行比对。两者严格相等，不存在冲突，因此类型绑定成功。
* **类型绑定**：
...
* **泛型实例化映射**：
    编译器在符号表中查找 `GenericArrayLoader` 的类定义。由于实例 `loader` 的 `T` 已被绑定为 `String`，编译器将泛型方法签名 `public T[] loadData(int size)` 映射为具体化签名：
...
**核心任务**：执行泛型擦除 (Type Erasure)、生成桥接逻辑与插入安全指令。
在此阶段，编译器将高层的泛型语义剥离，退化为 JVM 原生支持的类型系统。
...
**核心任务**：内存分配、JNI 调用与对象元数据校验。
此阶段验证了“利用反射传递类型令牌”方案的正确性。
