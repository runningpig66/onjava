...
在 2004 年 Java 5 发布泛型之前，Java 已经存在了 10 年。这 10 年间积累了海量的遗留代码（Legacy Code），所有的容器（List, Map 等）在底层都是基于 `Object` 实现的。

Sun 公司面临一个架构级的抉择：**如何在引入“泛型”这一新特性的同时，保持与现有 Java 生态系统的二进制兼容性（Binary Compatibility）？**
