 ...
 * @date 2月27日 周五
 * @time 10:14
 * P.193 §4.4 基于注解的单元测试 §4.4.2 实现 @Unit
...
public class AtUnit implements ProcessFiles.Strategy {
        ...
        for (Method m : testMethods) {
            ...
            try {
                ...
                try {
                    ...
                    // 此处专门捕获 InvocationTargetException，用于拦截测试方法内部抛出的异常（如断言失败）。
                    // 这样可以防止单个测试用例的失败导致整个测试流程意外终止，从而保证后续测试能够继续执行。
    ...
    private static Object createTestObject(Method creator) {
        if (creator != null) {
            try {
                // 实参 creator 对象经前置检查已确认是 static 方法。在反射调用 (invoke) 静态方法时，
                // 传入的第一个实例对象参数会被 JVM 忽略。因此此处传入 testClass 仅作为占位符，与传入 null 效果等同。
